...
![图 1](figures/fig01-token-waste.svg)
...
![图 2](figures/fig02-collapse.svg)

*图 2：同一次前向传播。上半保留了分布，能看出 0.58 和 0.40 挨得多近；下半 argmax 之后，这个「近」就再也看不见了。*
...
![图 3](figures/fig03-primitives.svg)
...
![图 5](figures/fig05-architecture.svg)

*图 5：据公开信息反推的结构。左边状态只有一份，选项那一路有 N 份；两路在交叉注意力处汇合，再分成三个头。浅色和虚线的部分都是推测。*

![图 6](figures/fig06-fanout-cost.svg)
...
![图 7](figures/fig07-time-budget.svg)
...
![图 8](figures/fig08-accuracy-vs-cost.svg)
...
![图 9](figures/fig09-proper-scoring.svg)
...
![图 10](figures/fig10-reliability.svg)

*图 10：我们的复现。上为 Jev，下为冻结 Qwen。浅色是原始曲线，两条都是 S 形、都偏离对角线；深色是等温回归之后，两条都贴回去了。曲线到对角线的竖直距离就是校准误差。*
...
![图 11](figures/fig11-effect-sizes.svg)

*图 11：上半是 mini-jev 的几个效应量，看点到零线的距离、区间有没有跨零：用字母做标签在 intent 上值 10 个点，在 domain 上值 13 个点，在布尔问题上一分不值；读 logits 对生成 JSON 差 0.22 个点，在噪声里。下半是 Banking77 上四个系统的准确率和置信区间。两组各用各的轴。*
